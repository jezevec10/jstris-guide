...
在进入网站后，所有玩家都会立即进入默认的**机器人房-Bot Room**，即机器人房间（俗称公房）。这样称呼这个房间是因为1个Bot（机器人）永远在线，位置就在你所有对手的左上角，还有着红色方块，在人群当中特别显眼易区分~
...
近日，Jstris引入了**地图-Map Downstack**模式。在该模式中，你可以在 *地图设计器-Map Designer* 中创建你自己的地图。一旦公开发布 (publish) 你的地图后，世界上的任何人都可以玩你的地图，并且互相竞争完成地图的最快时间。地图每日发表限制5幅、待发表队列限制10幅。每个地图都有排行榜，并且还有金、银、铜三个奖章分别对应第一、第二、第三名。在任何地图中获取前三名来赢得奖章并不断刷新自己的记录以保住你在排行榜上的位置吧！
...
在多人游戏中，垃圾行分配的方式共有4种。它们分别是：（后为使用指令）
...
ARR，全称Auto Repeat Rate，即自动重复比率。**但Jstris当中**这个数值是指**自动重复间隔长短**。如果这个数值调低，则会移动非常快，如果这个数值调高，自动重复的移动会非常的慢，过大甚至慢于连点移动键。Jstris的默认ARR值为0，即会在DAS结束后几乎立即撞到墙。如果感觉不适应可以尝试增大数值到合适为止。
